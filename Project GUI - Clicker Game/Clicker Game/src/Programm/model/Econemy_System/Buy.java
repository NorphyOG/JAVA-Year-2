...
public class Buy {
    ...
    public void buyFarmer(int pos, CubeKlicker cubeKlicker, InfoUpdate infoUpdate) {
        infoUpdate.getNummerFormart(BigInteger.valueOf(999));
        ...
        if (farmerModels[pos].getLevel() == 0 && farmerModels[pos].getPreis() <= cubes) {
            farmerModels[pos].setLevel(BigInteger.valueOf(1));
            farmerModels[pos].start();


            if(pos == 0) {
                textFuerStartkauf(pos = 0, cubeKlicker);
                model.getFarmerModels()[pos].setPreis(model.getFarmerModels()[pos].getPreis().multiply(BigInteger.valueOf(2)));
            ...
            if (pos == 0) {
                model.getFarmerModels()[0].setCubesPerSec((BigInteger) (model.getFarmerModels()[0].getCubesPerSec().add((BigInteger) BigInteger.valueOf((long) (multiplikatorStage1 + model.getPrestige()))).multiply(model.getFarmerModels()[0].getLevel())));
                textFuerInfo(pos = 0, cubeKlicker);
            } else if (pos == 1) {
                model.getFarmerModels()[1].setCubesPerSec((long) (model.getFarmerModels()[1].getCubesPerSec() + (multiplikatorStage2 + model.getPrestige()) * model.getFarmerModels()[1].getLevel()));
                textFuerInfo(pos = 1, cubeKlicker);
